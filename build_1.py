...
class TrainTwo(Train):
	...
	def build_model(self,train_data, train_labels, test_data,test_labels,layer_nodes):
		...
		model = Sequential()
		# The Input Layer :
		model.add(Dense(layer_nodes, kernel_initializer='normal',input_dim = train_data.shape[1], activation='relu'))
	...
	#	NN_model.add(Dense(256, kernel_initializer='normal',activation='relu'))
		...
		model.summary()
		#print layer_nodes[0]
		os.makedirs(str(layer_nodes))
		checkpoint_name = str(layer_nodes)+'/Weights-{epoch:03d}--{val_loss:.5f}.hdf5' 
		checkpoint = ModelCheckpoint(checkpoint_name, monitor='val_loss', verbose = 1, save_best_only = True, mode ='auto')
		callbacks_list = [checkpoint]

		history=model.fit(train_data, train_labels, epochs=250, batch_size=64, validation_data=(test_data,test_labels), callbacks=callbacks_list)
		...
		plt.ylim(0, 0.1)
		plt.xlabel('epoch')
		plt.legend(['test','train'], loc='upper left')    # why????????? which one is which
		plt.savefig(str(layer_nodes)+'/loss.png')
		plt.close()
